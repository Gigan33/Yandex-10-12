...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, Filter filter_document) const {
        const Query query = ParseQuery(raw_query);
        auto matched_documents = FindAllDocuments(query, filter_document);
        vector<Document> filter_documents;
        for (const auto& document : matched_documents) {
    
            if (filter_document(document.id, documents_.at(document.id).status, document.rating)) {
                filter_documents.push_back(document);
            }
        }
         sort(filter_documents.begin(), filter_documents.end(),
        ...
        if (filter_documents.size() > MAX_RESULT_DOCUMENT_COUNT) {
            filter_documents.resize(MAX_RESULT_DOCUMENT_COUNT);
        }
...
private:
    ...
    template <typename Filter>
    vector<Document> FindAllDocuments(const Query& query, Filter filter_document) const {
...
int main() {
    ...
    cout << "Even ids:" << endl;
    for (const Document& document : search_server.FindTopDocuments("пушистый ухоженный кот", [](int document_id, DocumentStatus status, int rating) { 
